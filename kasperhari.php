...
{	
	switch($_POST["jns2"])
	{case"Debet":$q=mysql_query("update kas set tgl='$_POST[tgl2]', transaksi='$_POST[tr2]', user='$_POST[user2]', donatur='$_POST[donatur2]', jenis='DEBET', jumlah='$_POST[jml2]' where no='$_POST[id2]'"); if($q){echo"<script>alert('alhamdulillah telah diubah')</script>";} else {echo"<script>alert('maaf belum diubah')</script>";} break;
	case"Kredit":$q=mysql_query("update kas set tgl='$_POST[tgl2]', transaksi='$_POST[tr2]', user='$_POST[user2]', donatur='$_POST[donatur2]', jenis='KREDIT', jumlah='$_POST[jml2]' where no='$_POST[id2]'"); if($q){echo"<script>alert('alhamdulillah telah diubah')</script>";} else {echo"<script>alert('maaf belum diubah')</script>";} break;}
...
<?php $qq=mysql_query("select *from kas where no='$_POST[id]'"); if($dit=mysql_fetch_array($qq))
?>    
    <tr>
      <td width="156">Tanggal</td>
    ...
    <tr>
      <td>Jumlah (Rp)</td>
      <td colspan="2"><input type="text" name="jml2" id="jml2" value="<?php echo $dit["jumlah"];?>"/></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
...
<table id="ss" width="100%" height="146" border="1" align="center">
  <tr align="center">
    <td height="26" colspan="2"><form id="form6" name="form6" method="post" action="">
      <input name="hari" type="date" id="hari" />
    <input type="image" src="foto/search.png" width="21" height="21" />
    </form></td>
    <td height="26">&nbsp;</td>
    <td height="26">&nbsp;</td>
    <td height="26">&nbsp;</td>
    ...
    <td colspan="2"><form id="input" name="input" method="post" action="">
      <input type="submit" name="Tambah" id="ss" value="Tambah" accesskey="T" />
    </form></td>
  </tr>
...
<?php if(empty($_POST["hari"]))$blnt=date('Y-m-d'); else $blnt=$_POST["hari"]; $q=mysql_query("select *from kas where tgl='$blnt' order by no asc"); while($kas=mysql_fetch_array($q)){
if($kas['jenis']=="DEBET"){
$debit=$kas['jumlah'];
$kredit="0";
$aldo=$aldo+$debit;
...
?>  
  <tr valign="middle">
    <?php if($kas["bon"]=='T') echo"<td height='22' bgcolor='#CC9900'> $kas[tgl]</td>"; else echo"<td height='22'> $kas[tgl]</td>"?>
...
<table width="100%" border="1" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td align="center">Tanggal <br /><?php  $now=date("Y-m-d"); $year=date("Y"); $mon=date("m");$hr=date("d");
...
if($hr > 1){
$sebelum = ($now - 1);
echo "<a href=$_SERVER[PHP_SELF]?tgltr=$sebelum>Prev</a>
";
}
...
echo "<a href=$_SERVER[PHP_SELF]?tgltr=$i>$i</a> ";
}
}
// bangun Next link
if($hr < $jumlahhari){
$selanjutnya = ($hr + 1);
echo "<a href=$_SERVER[PHP_SELF]?tgltr=$selanjutnya>Next</a>";
}
echo "</center>";
?></td>
  </tr>
